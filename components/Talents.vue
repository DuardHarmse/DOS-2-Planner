                    ...
                    <v-card-text class="pa-0">
                        <v-data-table v-model="selected" :headers="headers" :items="items" item-key="name" hide-actions select-all class="elevation-1">
                            <template slot="headers" scope="props">
                                <tr>
                                    <th v-for="header in props.headers" v-if="header.id != 'unspent_talents'" :key="header.text" @click="changeSort(header.value)" :class="getHeaderClass(header.align)">{{ header.text }}</th>
